...
class MockRIASECService(RIASECService):
    def __init__(self):
        pass

@pytest.fixture
def service():
    return MockRIASECService()

def test_single_code_classification(service):
    # Test 1: Single Code
    scores = {"R": 50, "I": 30, "A": 25, "S": 20, "E": 15, "C": 10}
    code, c_type, is_inconsistent = service.classify_riasec_code(scores)
    
    assert code == "R"
    assert c_type == "single"
    assert is_inconsistent is False

def test_dual_code_classification(service):
    # Test 2: Dual Code
...
def test_triple_code_classification(service):
    # Test 3: Triple Code
...
def test_inconsistent_profile(service):
...
if __name__ == "__main__":
    ...
    for tc in test_cases:
        code, c_type, inc = svc.classify_riasec_code(tc["scores"])
